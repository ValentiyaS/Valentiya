...
public class AppController {
    ...
    public ResponseEntity<?> handleGetRequest(
        ...
        if (id <= 10) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Invalid parameters: id must be > 10");
        }
        if (name.length() <= 4) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Invalid parameters: id must be > 5");
        }
        ...
        String getRequest = new String(Files.readAllBytes(Paths.get("Valentiya/src/main/resources/getRequest.txt")));
    ...
    public ResponseEntity<String> handlePostRequest(
        ...
        String postAnswer = new String(Files.readAllBytes(
                Paths.get("Valentiya/src/main/resources/postRequest.txt")));
